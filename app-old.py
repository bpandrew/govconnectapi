...
def login():
    ...
    try:
        ...
        if len(result)<=2:
            return functions.json_response('Fail', result)
        else:
            return functions.json_response('Success', result)
...
def user_add():
    ...
    result = UserSchema().dumps(user).data
    if len(result)>2:
        return functions.json_response('Error - email already exists', None)
    else:
        db.create_all()
...
def op():
    ...
    try:
        ...
        output['results_raw'] = OpSimpleSchema(many=True).dump(opportunities.items).data
...
def op_add():
    try:
        ...
        if obj==None:
            ...
            return functions.json_response('Success - Opportunity Already Exists', None)
    except Exception as e:
	    return(str(e))
...
def opportunities_delete(id_):
    try:
        ...
        db.session.delete(obj)
        db.session.commit()
        return functions.json_response('Success - Opportunity Deleted', None)
    except Exception as e:
	    return(str(e))
...
def addenda():
    try:
        ...
        result = AddendaSchema(many=True).dump(addenda).data
        return jsonify(result)
    except Exception as e:
	    return(str(e))
...
def addenda_detail(addenda_id):
    try:
        ...
        result = AddendaSchema().dumps(addenda).data
...
def addenda_add():
    ...
    response = AddendaSchema().dump(addenda).data
    return functions.json_response('Success - Added Addenda', response)
...
def contracts():
    ...
    if paginate=="yes":
        ...
        output['pagination'] = {"next_url":next_url, "prev_url":prev_url, "total":total, "pages":pages}
        output['results'] = ContractSchema(many=True).dump(contracts.items).data
    else:
        contracts = query.all()
...
def contract_detail(contract_id):
    try:
        ...
        result = ContractSchema().dumps(contract).data
...
def unspsc_detail(unspsc_id):
    ...
    result = UnspscSchema().dumps(unspsc).data
    result = json.loads(result)

    unspsc = Unspsc.query.filter_by(parent_id=unspsc_id).order_by(Unspsc.title).all()
    children = UnspscSchema(many=True).dump(unspsc).data
...
def aps():
    ...
    try:
        ...
        result = EmployeeSchema(many=True).dump(employee).data
        return jsonify(result)
    except Exception as e:
	    return(str(e))